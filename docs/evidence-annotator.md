...
```text
pi --mode json --print --no-session --no-tools --no-extensions --no-skills ...
```

Pi annotator 没有 session、工具、extension、skill、prompt template 或 context file。
模型和 provider 由固化 profile 决定，provider 配置从 `PI_CODING_AGENT_DIR` 读取。
