...
- Pi 的 wait-any、best 观察和跨进程 native-session continuation；
- Codex/Pi 真机 cycle 与报告证据。

AtomicPlan、共享 Search State、admission 和 EvidenceCommit 不在本次实现范围内，仍由
对应的外部工作补齐。Pi continuation 会启动新进程，但从
`.gp/host-sessions/pi/` 恢复相同 native session，并保持相同 `agent_session_id`、
candidate 和 workspace。
...
- Codex 可以通过 `search_continue_agent_session` + `followup_task` 继续原生
  subagent；
- Pi 可以通过 `pi_search_pool_continue` 恢复相同 native session 和 candidate/workspace；
  启动新的无 session Pi 进程。
...
Pi 的 `pi_search_pool_continue` 会对相同 candidate/workspace 执行兼容入口
`redispatch=true`，但内部调用 `search_continue_agent_session`，启动新的 Pi 进程并
加载相同 native session。runtime `agent_session_id`、candidate 和 workspace 均保持不变。

这属于**跨进程 native-session continuation**。它不保证 PID 不变；若产品未来明确要求
同一 OS 进程在整个外层预算内长驻，才需要额外的 persistent Pi supervisor。该能力不应
与 native session continuity 混为一谈。
...
当前实现使用 persisted Pi session 和增量 entries cursor，在新进程中恢复同一原生
session。只有明确要求同 PID 长驻时，才新增 persistent RPC supervisor。
...
- Pi resume 显示为同一 native session 下的多个 process dispatch segment；
...
agent session = one native conversational session
process dispatch = one bounded host execution segment
...
### S5：Pi Native Session Resume

- `pi_search_pool_continue` 复用相同 native session 和 `agent_session_id`；
- 每次 dispatch 启动新进程，并用 entries cursor 只采集新增指标；
- persistent same-PID supervisor 仅在进程身份成为明确需求时实现。
