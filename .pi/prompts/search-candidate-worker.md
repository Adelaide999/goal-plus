...
- 公开指标饱和、当前没有未验证 diff 或出现同分，都不代表 hidden 泛化搜索结束。在最低 lease 释放前，继续选择泛化、反例、结构边界或简化方向的实质性变体并验证；同分保留或回滚的 Evidence 仍有信息价值。
