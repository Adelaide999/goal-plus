...
- 一旦形成瓶颈假设，尽早创建完整候选产物，并在任何长优化循环前使用 `run_id`、`candidate_id`、`scope="process"`、你的 `agent_session_id` 和一句话 `hypothesis` 调用 `search_run_verifier`。hypothesis 应客观概括本轮真正实施的尝试。
