...
- 首次修改前调用 `search_get_global_evidence(agent_session_id)`。默认结果是有界索引：每个 candidate 的普通 View 最多包含 `hard_best` 和 `latest_settled` 两个代表项，并注明完整 Evidence 数量和省略数量。此后每完成 3 次 `search_run_verifier` iteration 刷新一次；连续两轮没有提升或准备切换技术路线时提前刷新。若 verifier 返回 `global_evidence_injected=true`，其中的 `global_evidence_snapshot` 已完成本次刷新。仅当默认索引不足时，使用 `search_list_global_evidence` 分页浏览轻量历史引用，再用 `search_get_global_evidence_entry` 按精确 candidate/iteration/commit 展开一条完整 View；不要批量展开全部历史。`context.supplemental_evaluation_enabled=false` 时不要读取补充评价；启用时对 `supplemental_available=true` 的行按需调用一次 `search_get_evidence_detail`。detail 返回 supported/unresolved observations；`observation_ordinal` 与 candidate/iteration/commit 组成精确引用。需要比较时可先调用 `search_list_evidence_topics`，再调用 `search_compare_evidence` 显式选择 2–8 条 observation（每 candidate 最多 2 条），或让 topic 模式按 candidate_id 选取最新 supported 与最新 unresolved。比较不使用硬 score，也不是 hidden 结果、排名、推荐或 promotion gate。`view=null` 不影响 Evidence，无需等待或轮询。
