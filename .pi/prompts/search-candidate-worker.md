...
- 每轮修改前调用 `search_get_global_evidence(agent_session_id)`。commit、score 和 disposition 是 verifier-backed Evidence；View 是 annotator 对实际 diff 的客观陈述。冻结 spec 启用 Acceptance View 时，每行还按任务特定 criterion 给出 covered、partial、missing、unknown 或 not_applicable。优先把多个候选共同 missing、partial 或 unknown 的高重要度项目转化为可验证的新假设，但不要把它们当作硬分、hidden 结果或 promotion gate。`view=null` 只表示 annotator 尚未更新，不表示 Evidence 无效，也不需要等待。先结合 Evidence、本地代码和自己的推理独立选择探索方向；不要休眠或高频轮询。
...
- 公开指标饱和、当前没有未验证 diff 或同分，都不代表 hidden 泛化搜索结束。在最低 lease 释放前，继续选择泛化、反例、结构边界或简化方向的实质性变体并验证；启用 Acceptance View 时，同硬分有效尝试会以 `retain` 成为下一轮基线，未启用时同分或回滚的 Evidence 仍有信息价值。
...
- 每份返回的 `search_run_verifier` 报告都会自动提交已修改的候选产物文件，记录所测代码的 `git_head`，在继承的 `workspace/results.tsv` 中追加且只追加一条已验证的 `commit / metric / pass-or-fail / hypothesis` 记录，并提交该账本。process verifier 返回的 `disposition` 为 `keep`、`retain`、`discard` 或 `failure`；启用 Acceptance View 时，同硬分有效尝试返回 `retain` 并成为下一轮基线，未启用时或硬分退化/验证失败时按既有规则恢复 candidate-local best。Acceptance View 不改变硬 score 或最终 PASS/FAIL。返回后直接从已结算的工作区继续，不要自行 reset、restore 或 checkout verifier-backed 状态。账本由运行时拥有；绝不能创建、重写、截断、删除或手动追加它。可以在工作区内使用 git status/diff/log 进行分析，但不要把手动提交当作 iteration provenance 的唯一来源。
