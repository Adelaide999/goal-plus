...
- 如果 `candidate_task.share_out_dir` 非空，说明本 run 已启用 `shared_dir`。工具化的目标是降低同一 run 内其他 candidate 重建诊断或检查流程的成本，不要求跨项目通用。每次 verifier 前回顾本轮及此前 iteration 的命令序列、临时代码片段和 scratch scripts；只要工具能在 peer workspace 运行、不依赖当前 candidate 的临时私有状态，并命中至少一个正向信号，就默认提炼为最小工具并 stage：`repeated_sequence` 表示相同或等价的多步流程已出现至少两次；`domain_probe` 表示含非显然领域对象构造、边界条件或断言；`parser_or_trace` 表示解析、trace 收集、复现、转换或 mutation 检查；`peer_setup_reduction` 表示能明显减少 peer 重建诊断环境或检查流程的成本。
- 推荐先把源文件保存在 `.tmp/tool-drafts/`，再调用 `search_stage_shared_tool(agent_session_id, name, summary, entrypoint, candidate_relative_source_paths)` 生成 `.tmp/share-out` staging。不要因为工具短、任务专属、来自临时代码片段或只产生退出码而拒绝发布。只有以下具体排除项才支持 `not_applicable`：`single_common_command`（只有单条普通命令）、`logic_free_wrapper`（无自定义逻辑的普通命令包装）、`restricted_artifact`（主产物、candidate 测试、冻结 verifier/grader、日志、数据、凭据或构建输出）、`candidate_private_state`（绝对路径、未提交私有状态或无法在 peer workspace 运行）或 `duplicate_snapshot`（与已发布快照完全相同）。如果 producer 后续仍要使用工具，应保留工作副本，因为 verifier 会消费 staging。
- 每次归属于本 worker 的 process verifier 调用都传入 `toolization_decision`。`outcome=staged` 必须列出至少一个上述 `signals` 和实际 `tool_names`；`outcome=not_applicable` 必须给出具体 `exclusion`，不能只写“不可复用”。runtime 以 staging inventory 和 publication settlement 为权威；缺失决策、声明 staged 但 staging 为空、或声明 not_applicable 但 staging 非空只会生成 monitor/report advisory，不改变 score、disposition、selection 或 promotion。`shared_tool_publish_status=not_staged` 只说明该轮 staging 为空，不是 runtime 或 annotator 故障。
- 重新派发或处于继承的子/后继工作区时，在判断剩余工作前检查 `context.resume.latest_handoff`、先前 session 摘要、`context.results`、`context.results_tsv` 和当前工作区状态。
- 每轮修改前调用 `search_get_global_evidence(agent_session_id)`。commit、score 和 disposition 是 verifier-backed Evidence；View 是 annotator 对实际 diff 的客观陈述。`context.supplemental_evaluation_enabled=false` 时不要读取补充评价；启用时仅在路线停滞、分数跃升或本地/外部结果背离等需要深挖的情况下，对 `supplemental_available=true` 的行调用一次 `search_get_evidence_detail`。补充评价不来自 FrozenSpec，也不是硬分、hidden 结果、推荐或 promotion gate。`view=null` 不影响 Evidence，无需等待或轮询。
- 作为采用方，peer 工具只会在 annotator 生成且 runtime 绑定的 `shared_tools[*].tool_view` 中出现。先独立判断相关性，再用 `search_copy_shared_tool(agent_session_id, tool_id, snapshot_hash)` 复制精确快照到本地临时 inbox 并重新验证；下一次 verifier 原子消费 receipt。工具 View 与采用结果是后续搜索的参考，不改变 hard score、retain/discard、选择或 promotion。
