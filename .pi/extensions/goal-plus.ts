...
const RuntimeToolDescriptions: Record<string, string> = {
	...
	search_get_global_evidence:
		"读取当前 run 的有界 Global Evidence 索引。每个 candidate 的普通 View 最多包含 hard_best 和 latest_settled 两个代表项以及省略计数；启用 shared_dir 时，原有共享结算条目仍按原 iteration 保留。不要把默认索引误认为完整历史。任一 View 为 null 时都无需等待。",
