...
const RuntimeToolDescriptions: Record<string, string> = {
	...
	search_run_verifier:
		"为一个候选评分。worker process verifier 必须提供一句话 hypothesis，客观概括本轮实际尝试。每份返回的 verifier 报告都会在运行时拥有、继承而来的 workspace/results.tsv 中追加且只追加一条已验证记录，并提交该文件。process verifier 返回 keep/retain/discard/failure disposition；严格改善为 keep，同分为 retain 并成为 candidate-local 最新基线，只有退化或验证失败时恢复此前硬分最佳。global_evidence_injected=true 时，global_evidence_snapshot 是本轮结算后自动读取的共享 Evidence/View 快照，应作为下一轮假设的输入并与本地代码独立核对；它同时生成可审计 read receipt，但不替代下一轮编辑前的刷新。开放式补充评价和动态 peer 比较不改变结算、硬 score 或最终 PASS/FAIL。带 candidate_action=stop_and_report 的 VerifierWorkspaceSideEffect 属于基础设施失败：worker 必须停止，不能清理或重试，使父级能够修复并重新冻结。",
